...
export default function ModalContent({ props, change }) {
  let ctx = useContext(MovieWatchListContext);
  //Destructure the props
  const { name, image, description } = props;

  //Rename the summary
  let summary = description.substring(
  ...
  let styles = {
    modalStyle:
      "flex flex-col gap-8 w-[500px] md:w-[816px] md:h-[532px] bg-slate-600 p-9 text-white font-bold",
    modalImage: "w-40 h-60 bg-slate-300",
    modalAlignment: "flex flex-row gap-5",
    paragraph: "text-xl font-normal mt-3",
  };

  return (
    <div className={styles.modalStyle}>
      <figure className={styles.modalAlignment}>
        <img className={styles.modalImage} src={image.medium} alt="" />
        <figcaption>
          <article className="">
      ...
      <div>
      <Span className={"block text-2xl"} text={"Because of your interest in:"}/>
        ...
        <div className="flex flex-row gap-3 w-full mt-1">
          <button
            className="w-10/12 text-2xl "
            onClick={() => ctx.addMovieToWatchlist(props)}
          >
            <span className="mr-5">
              <svg
